...
- Khi đổi màu nét/tô tại một đối tượng bất kỳ thuộc nhóm công tác (`workItemId` bao gồm đối tượng chính và các phần đo bổ sung), toàn bộ các đối tượng thành phần trong nhóm sẽ tự động chuyển màu đồng bộ tức thì trên bản đồ và được lưu đồng bộ xuống cơ sở dữ liệu.
...
- **Chế độ Kính lúp điểm chấm 2x (`TouchMagnifierGlass`)**: Hỗ trợ phóng đại 2x trên mobile/touch và desktop khi bật icon `🔍 Magnifier`. Tự động duy trì khung hình WebGL (`triggerRepaint`) kết hợp bộ đệm dự phòng 2D (`offscreenCanvas`), đảm bảo hình ảnh phóng to 2x luôn luôn hiển thị siêu rõ nét 100% kể cả khi di chuyển chuột hay khi dừng hẳn con trỏ đứng yên trên bản đồ.
...
- Giai đoạn 1 hoàn tất ngày 26/07/2026: toolbar ở chế độ xem chỉ giữ **Điểm, Chiều
  ...
  và Hủy. Kính lúp cùng **Mở nâng cao** nằm trong menu **Thêm công cụ**. Nền bản
  đồ, ranh giới xã/phường và ba chế độ thực địa chỉ giữ trên header, không lặp lại
