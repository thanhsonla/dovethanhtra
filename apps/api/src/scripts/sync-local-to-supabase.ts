...
if (!remoteUrl) {
  throw new Error('Missing required environment variable: REMOTE_DATABASE_URL')
}
...
async function main(): Promise<void> {
  ...
  async function syncTable(table: string, transformRow?: (row: BaseRecord) => void): Promise<void> {
  ...
  for (const [measurementId, supersedesId] of measurementSupersedesMap.entries()) {
    await remotePool
      .query(`UPDATE "measurement" SET supersedes_id = $1 WHERE id = $2`, [
        supersedesId,
        measurementId,
      ])
      .catch((err) => {
        const errMsg = err instanceof Error ? err.message : String(err)
        console.error(`⚠️ Restoring measurement supersedes_id (${measurementId}):`, errMsg)
      })
  }
  ...
  for (const [measurementId, draftId] of measurementDraftMap.entries()) {
    await remotePool
      .query(`UPDATE "measurement" SET capture_draft_id = $1 WHERE id = $2`, [
        draftId,
        measurementId,
      ])
      .catch((err) => {
        const errMsg = err instanceof Error ? err.message : String(err)
        console.error(`⚠️ Restoring measurement capture_draft_id (${measurementId}):`, errMsg)
      })
  }
  ...
  for (const row of localDrafts.rows) {
    if (row.id && row.status === 'classified' && row.classified_measurement_id) {
      await remotePool
        ...
        .catch((err) => {
          const errMsg = err instanceof Error ? err.message : String(err)
          console.error(`⚠️ Restoring capture_draft classified status (${String(row.id)}):`, errMsg)
        })
    }
  }
  ...
  console.log('🎉 ĐÃ ĐỒNG BỘ THÀNH CÔNG 100% DỮ LIỆU ĐO LOCAL LÊN SUPABASE VÀ VERCEL!')
}
