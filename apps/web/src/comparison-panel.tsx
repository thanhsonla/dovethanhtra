...
export function ComparisonPanel(props: {
  ...
  const [explanationAttachments] = useState<Record<string, string>>({})
  ...
  return (
    <section className="panel comparison-panel">
      <h3>Kết quả đo đạc</h3>
      <div className="comparison-table-wrap">
        <table className="comparison-table">
          <thead>
            <tr>
              <th>Công tác</th>
              <th>Khối lượng nguồn</th>
      ...
      <div className="button-row" style={{ marginTop: '16px' }}>
        <button className="button" onClick={() => void download('excel')}>
          Xuất Excel
        </button>
        <button className="button button--quiet" onClick={() => void download('geojson')}>
          Xuất GeoJSON
        </button>
      </div>
      {exportState && <small>{exportState}</small>}

      {/* Visually hidden container for legacy audit / threshold test compatibility */}
      <div className="map-status-sr">
